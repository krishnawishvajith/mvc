...
<div class="main-content">
    <div class="dashboard-header">
        <h1>Stadium Owner Package Management</h1>
    ...
    <form id="packages-form">
        <div class="packages-grid">
            <?php
            $packageIds = array_column($data['packages'], 'id');
            foreach ($data['packages'] as $pkg):
                $id = $pkg['id'];
                $typeClass = htmlspecialchars($pkg['color'] ?? 'standard');
            ?>
                <div class="package-card <?php echo $typeClass; ?> <?php echo $pkg['is_popular'] ? 'popular' : ''; ?>">
                    <div class="package-header">
                        <h3><?php echo htmlspecialchars($pkg['name']); ?> Package</h3>
                        <label>
                    ...
                    <div class="form-grid">
                        ...
                        <input type="hidden" name="package_<?php echo $id; ?>_id" value="<?php echo $id; ?>">
                        ...
                        <div class="form-group-small">
                            <label>Setup Fee (LKR)</label>
                            <input type="number" name="package_<?php echo $id; ?>_setup_fee" value="<?php echo $pkg['setup_fee']; ?>" class="form-control" step="0.01">
                        </div>

                        <div class="form-group-small">
                            <label>Commission (%)</label>
                            <input type="number" name="package_<?php echo $id; ?>_commission" value="<?php echo $pkg['commission_rate']; ?>" class="form-control" min="0" max="100">
                        </div>

                        <div class="form-group-small">
                            <label>Stadium Limit</label>
                            <input type="text" name="package_<?php echo $id; ?>_stadiums" value="<?php echo $pkg['stadium_limit']; ?>" class="form-control" placeholder="3 or 'unlimited'">
                        </div>

                        <div class="form-group-small">
                            <label>Photos per Stadium</label>
                            <input type="number" name="package_<?php echo $id; ?>_photos" value="<?php echo $pkg['photos_limit']; ?>" class="form-control" min="1" max="20">
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Support Level</label>
                        <input type="text" name="package_<?php echo $id; ?>_support" value="<?php echo htmlspecialchars($pkg['support_level'] === 'email' ? 'Email Support' : ($pkg['support_level'] === 'phone' ? 'Email & Phone Support' : 'Priority Support 24/7')); ?>" class="form-control">
                    </div>
        ...
        <input type="hidden" name="package_ids" value="<?php echo htmlspecialchars(implode(',', $packageIds)); ?>">
    </form>

    <!-- Package Purchases Section -->
    <div class="purchases-section">
        <div class="section-header">
            <h2>📦 Package Purchase Management</h2>
        ...
        <?php if (empty($data['package_purchases'])): ?>
            <div class="no-purchases">
                <p>📭 No package purchases yet. Users will appear here after purchasing packages.</p>
            </div>
        <?php else: ?>
                            ...
                            <tr class="purchase-row" data-purchase-id="<?php echo $purchase->id; ?>">
                                ...
                                <td>
                                    <div class="business-info">
                                        ...
                                        <?php endif; ?>
                                        <?php if (!empty($purchase->venue_type)): ?>
                                            <small>🏟️ <?php echo htmlspecialchars(ucwords(str_replace('_', ' ', $purchase->venue_type))); ?></small>
                                        <?php endif; ?>
                                    </div>
                                </td>
                                <td>
...
<script>
    ...
    function updatePurchaseStatus(purchaseId, newStatus) {
        ...
        formData.append('action', 'update_purchase_status');
...
<style>
    ...
    .package-card.popular {
        box-shadow: 0 0 20px rgba(3, 178, 0, 0.2);
    }
